...
function getMissingTools(material: string) {
    let missingTools = [];
...
async function gatherPlanks() {
    ...
    if (!planks || planks.count < 3) {
        ...
        let recipe = bot.recipesFor(plankType.id, log.metadata, null, null)[0];
...
async function gatherSticks() {
    ...
    if (!sticks || sticks.count < 2) {
        ...
        let recipe = bot.recipesAll(stickItem.id, null, null)[0];
        console.log(recipe);
        await bot.craft(recipe);
    }
...
export const PrepareToolsCommand = {
    minArgs: 0,
    maxArgs: 1,

    async handler(material = "wooden") {
